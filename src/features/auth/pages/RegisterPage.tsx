...
import { selectAuthError, selectIsAuthenticated, setError } from "../slices/authSlice"
...
export default function RegisterPage() {
  ...
  const handleRegister = async (formData: FormFields) => {
    ...
    try {
      ...
      if (err && typeof err === "object" && "data" in err) {
        ...
        dispatch(setError(errorData.message || "Registration failed. Please try again."))
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#727a75] flex items-center justify-center p-4">
      <div className="w-full max-w-6xl bg-gray-900 rounded-2xl overflow-hidden flex flex-col md:flex-row shadow-xl">
     
        <div className="w-full md:w-1/2 relative bg-emerald-900/50 text-white">
          <div className="absolute top-6 left-6">
            <Link to="/" className="flex items-center gap-2">
              <EwateLogo width={40} height={40} />
              <span className="text-xl font-bold">E-Waste</span>
            </Link>
          </div>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 bg-white  text-black">
          <div className="max-w-md mx-auto">
            ...
            <AuthForm type="register" onSubmit={handleRegister} isLoading={isLoading} error={authError} />

            <div className="mt-8 relative flex items-center justify-center">
              <hr className="w-full border-gray-700" />
              <span className="absolute bg-gray-900 px-4 text-sm text-gray-400">Or register with</span>
            </div>
